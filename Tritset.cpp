...
size_t Tritset::shrink() {
    ...
    return cut(static_cast<size_t>(_last));
}
...
void Tritset::reference::set_value(const Trit &value) {
    ...
    set->array[id] &= ~(static_cast<unsigned>(trit_pack::mask) << shift);
    if (value == trit::True)
        set->array[id] |= static_cast<int>(trit_pack::True) << shift;
    else if (value == trit::Unknown)
        set->array[id] |= static_cast<int>(trit_pack::Unknown) << shift;
    else
        set->array[id] |= static_cast<int>(trit_pack::False) << shift;

    if ((int) index > set->_last && value != trit::Unknown)
        set->_last = static_cast<int>(index);
}
...
Trit Tritset::operator[](const int index) const {
    ...
    if (value == static_cast<unsigned>(trit_pack::False))
        return Trit(false);
    if (value == static_cast<unsigned>(trit_pack::True))
        return Trit(true);
    return Trit(trit::Unknown);
}
...
Tritset::cell_t Tritset::_fill_value(trit val) {
    ...
    switch (val) {
        case trit::False:
            elem = static_cast<unsigned>(trit_pack::False);
            break;
        case trit::True:
            elem = static_cast<unsigned>(trit_pack::True);
            break;
        case trit::Unknown:
            elem = static_cast<unsigned>(trit_pack::Unknown);
            break;
    }
    cell_t res = cell_t(0);
...
Tritset::iterator Tritset::end() {
    return Tritset::iterator(this, static_cast<size_t>(_last + 1));
}
